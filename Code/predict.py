...
WEIGHTS_PATH = "C:/Users/admitos/Desktop/ThesisUU/pretrained_models/coco_weights_mlp.pt"
IMAGES_PATH = "F:/ThesisUU/val/"
...
class Predictor():
    ...
    def predict(self, image, model, use_beam_search):
        ...
        image = self.preprocess(pil_image).unsqueeze(0).to(CPU)
        with torch.no_grad():
            prefix = self.clip_model.encode_image(image).to(CPU, dtype=torch.float32)
...
predictor = Predictor()
my_model = predictor.setup()
generated_caps = []
for idx,filename in enumerate(os.listdir(IMAGES_PATH)):
    if filename.endswith(".jpg"):
        my_img_path = os.path.join(IMAGES_PATH,filename)
        generated_caps.append(predictor.predict(my_img_path, my_model, USE_BEAM_SEARCH))
        print("For the image: {}, the generated caption is: {}".format(filename,generated_caps[idx]))
        if idx>1:
            break
