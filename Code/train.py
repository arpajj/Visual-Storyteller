...
class ClipVistDataset(Dataset):
    ...
    def __init__(self, data_path: str,  prefix_length: int, gpt2_type: str="gpt2", normalize_prefix=False):
        ...
        print("Data size is", len(test_embeddings['captions']))
        sys.stdout.flush()
        self.prefixes = test_embeddings["clip_embedding"]
        captions_raw = test_embeddings["captions"]
        self.image_ids = [caption["photo_flickr_id"] for caption in captions_raw]
        self.captions = [caption['description'] for caption in captions_raw]
...
def train(dataset: ClipVistDataset, model: ClipCaptionModel, args, lr: float=2e-5, warmup_steps: int=1000, output_dir: str = ".", output_prefix: str = ""):
    ...
    print("The length of the dataloader is: ", len(train_dataloader))
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', default='C:/Users/admitos/Desktop/ThesisUU/CLIP_image_embeddings/test.pkl')
    ...
    parser.add_argument('--only_prefix', dest='only_prefix', action='store_true') # 'store_true' trains only the prefix. 'store_false' trains both the prefix and GPT
    ...
    dataset = ClipVistDataset(args.data, prefix_length, normalize_prefix=args.normalize_prefix)
